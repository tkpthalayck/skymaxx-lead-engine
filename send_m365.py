...
<p>We help organizations like yours streamline their <strong>Microsoft 365 / Office 365 management</strong> &mdash; typically saving 15-20 hours per week of admin overhead while improving security posture and reducing licensing costs by 25-40%.</p>
...
print('Loaded ' + str(len(leads)) + ' leads')

log = []
log.append('=== M365 Campaign  Step 1 (Intro) ===')
log.append('Total leads: ' + str(len(leads)))
log.append('From: ' + FROM_EMAIL)
log.append('Reply-to: ' + REPLY_TO)
log.append('')
...
for i, lead in enumerate(leads, 1):
    ...
    ok, info = send(email, lead.get('name', ''), subject, body)
    company = lead.get('company', '')[:30]
    if ok:
        success += 1
        line = '[' + str(i) + '/' + str(len(leads)) + '] ' + email + ' | ' + company + ' | OK SENT'
    else:
        failed += 1
        line = '[' + str(i) + '/' + str(len(leads)) + '] ' + email + ' | ' + company + ' | FAIL ' + info[:100]
    print(line, flush=True)
    log.append(line)
    time.sleep(1.5)

log.append('')
log.append('=== Summary: ' + str(success) + ' sent, ' + str(failed) + ' failed ===')
print('Total: ' + str(success) + ' sent, ' + str(failed) + ' failed')
